...
router.get('/', async (_req, res, next) => {
  try {
    res.json(await User.find({ role: 'trainer', isEmailVerified: true }).select(trainerFields).sort({ name: 1 }));
...
router.get('/:id', async (req, res, next) => {
  try {
    ...
    const trainer = await User.findOne({ _id: req.params.id, role: 'trainer', isEmailVerified: true }).select(trainerFields).lean();
