  ...
  return <>
    <section className="home-hero">
      <div className="hero-copy">
        ...
        <p>FitFlow turns classes, coaching, and workout tracking into one calm weekly command center, so your routine feels planned before motivation has to fight for it.</p>
    ...
    <section className="home-flow">
      ...
      <div className="flow-grid">
        <Feature number="01" title="Pick your signal" text="Explore live classes by category, level, coach, schedule, and open spots." icon="calendar" />
...
function Classes() {
  ...
  return <section className="page section"><div className="page-head"><div><div className="eyebrow dark"><span />Move your way</div><h1>Find a class<br/><em>worth showing up for.</em></h1></div><p>Small groups, thoughtful coaching, and a level for every starting point.</p></div>
    ...
    {loading ? <SkeletonGrid count={4} className="class-grid" /> : <div className="class-grid">{shown.map((item, index) => <article className="class-card" key={item._id} style={{'--accent': item.accent}}>
      ...
      <div className="class-body"><div className="class-meta"><span>{item.level}</span><span><Icon name="clock" size={15}/>{item.duration} min</span></div><h2>{item.title}</h2><p>with {item.trainer ? <Link to={`/trainers/${item.trainer}`}>{item.coach}</Link> : item.coach}</p><div className="class-footer"><div><small>{item.schedule}</small><b>{item.spotsLeft} spots left</b></div><button onClick={() => book(item._id)}>Book <Icon name="arrow" size={17}/></button></div></div>
    </article>)}</div>}
  </section>;
}
...
function TrainerProfile() {
  ...
  return <section className="trainer-page section">
    <div className="trainer-hero"><div><div className="eyebrow dark"><span />Coach profile</div><h1>{trainer.name}</h1><p>{profile.headline || 'FitFlow coach focused on practical, repeatable progress.'}</p></div><Link className="button primary" to="/classes">Book a class <Icon name="arrow" /></Link></div>
    <div className="trainer-layout"><article className="panel trainer-bio"><h2>Coaching style</h2><p>{profile.bio || 'This trainer keeps sessions clear, encouraging, and adaptable to real-life schedules.'}</p><div className="tag-row">{(profile.specialties?.length ? profile.specialties : ['Strength', 'Consistency', 'Mobility']).map(item => <span key={item}>{item}</span>)}</div>{Boolean(profile.certifications?.length) && <p className="certs"><b>Certifications:</b> {profile.certifications.join(', ')}</p>}</article>
...
function Coach() {
  ...
  return <section className="coach-page section"><div className="page-head"><div><div className="eyebrow dark"><span/>Adaptive Weekly Coach</div><h1>A plan built for<br/><em>your real week.</em></h1></div><p>Your coach combines your goal, availability, recent activity and upcoming classes. It adapts without judging missed days.</p></div>
    <div className="coach-layout"><form className="panel coach-form" onSubmit={generate}><span className="form-kicker">TELL YOUR COACH</span><h2>Make this week realistic</h2><label>Experience<select name="experience" defaultValue={user.profile?.experience}><option>Beginner</option><option>Intermediate</option><option>Advanced</option></select></label><label>Minutes per session<input name="preferredMinutes" type="number" min="10" max="180" defaultValue={user.profile?.preferredMinutes || 30}/></label><fieldset><legend>Available days</legend><div className="day-picker">{days.map(day => <label key={day}><input type="checkbox" name="availableDays" value={day} defaultChecked={user.profile?.availableDays?.includes(day)}/><span>{day.slice(0,3)}</span></label>)}</div></fieldset><label>Equipment<input name="equipment" defaultValue={user.profile?.equipment?.join(', ')} placeholder="Dumbbells, bands, treadmill"/></label><label>Weight in kg <small>(for calorie estimates)</small><input name="weightKg" type="number" min="25" max="350" defaultValue={user.profile?.weightKg}/></label><label>Limitations or preferences<textarea name="limitations" defaultValue={user.profile?.limitations} placeholder="Keep this general; speak to a professional about medical concerns."/></label><button className="button primary full" disabled={busy}>{busy ? 'Building your week...' : plan ? 'Regenerate my week' : 'Build my week'}<Icon name="bolt"/></button></form>
...
function Admin() {
  ...
  return <section className="dashboard section"><div className="dash-head"><div><p>{user.role.toUpperCase()} WORKSPACE</p><h1>Run FitFlow clearly.</h1></div></div>{overview && <div className="stats-grid"><Stat label="Members" value={overview.members}/><Stat label="Classes" value={overview.classes}/><Stat label="Workouts logged" value={overview.workouts}/></div>}<div className="admin-tools"><input value={query} onChange={e => setQuery(e.target.value)} placeholder="Search users, classes, coaches..." />{user.role === 'admin' && <select value={roleFilter} onChange={e => setRoleFilter(e.target.value)}><option value="all">All roles</option><option value="member">Members</option><option value="trainer">Trainers</option><option value="admin">Admins</option></select>}</div><div className="admin-grid"><form className="panel admin-form" onSubmit={addClass}><span className="form-kicker">PUBLISH A SESSION</span><h2>Add a real class</h2><div className="form-row"><label>Title<input name="title" required/></label><label>Category<input name="category" required/></label></div><div className="form-row"><label>Coach<input name="coach" defaultValue={user.name} required/></label><label>Starts at<input name="startsAt" type="datetime-local" required/></label></div><div className="form-row"><label>Duration<input name="duration" type="number" defaultValue="45" required/></label><label>Capacity<input name="capacity" type="number" defaultValue="16" required/></label></div><div className="form-row"><label>Level<select name="level"><option>Beginner</option><option>Intermediate</option><option>Advanced</option></select></label>{user.role === 'admin' && <label>Trainer<select name="trainer"><option value="">Unassigned</option>{trainers.map(trainer => <option value={trainer._id} key={trainer._id}>{trainer.name}</option>)}</select></label>}</div><label>Display schedule<input name="schedule" placeholder="Mon - 6:30 PM" required/></label><button className="button primary full">Publish class</button></form>{user.role === 'admin' && <article className="panel"><div className="panel-title"><div><span>ACCESS CONTROL</span><h2>Team and members</h2></div></div><div className="user-list">{filteredUsers.length ? filteredUsers.map(item => <div key={item._id}><div><b>{item.name}</b><span>{item.email}</span></div><select value={item.role} onChange={e => changeRole(item._id,e.target.value)}><option>member</option><option>trainer</option><option>admin</option></select></div>) : <Empty text="No people match that search." link="Browse classes" />}</div></article>}</div>
    <article className="panel class-manager"><div className="panel-title"><div><span>CLASS OPERATIONS</span><h2>{user.role === 'trainer' ? 'Your sessions' : 'All sessions'}</h2></div></div><div className="managed-classes">{filteredClasses.length ? filteredClasses.map(item => <form key={item._id} data-id={item._id} onSubmit={updateClass}><div className="form-row"><label>Title<input name="title" defaultValue={item.title} required/></label><label>Category<input name="category" defaultValue={item.category} required/></label></div><div className="form-row"><label>Coach<input name="coach" defaultValue={item.coach} required/></label><label>Starts at<input name="startsAt" type="datetime-local" defaultValue={localDate(item.startsAt)} required/></label></div><div className="form-row"><label>Duration<input name="duration" type="number" defaultValue={item.duration} required/></label><label>Capacity<input name="capacity" type="number" defaultValue={item.capacity} required/></label></div><div className="form-row"><label>Level<select name="level" defaultValue={item.level}><option>Beginner</option><option>Intermediate</option><option>Advanced</option></select></label>{user.role === 'admin' && <label>Trainer<select name="trainer" defaultValue={item.trainer?._id || item.trainer || ''}><option value="">Unassigned</option>{trainers.map(trainer => <option value={trainer._id} key={trainer._id}>{trainer.name}</option>)}</select></label>}</div><label>Display schedule<input name="schedule" defaultValue={item.schedule} required/></label><div className="manager-actions"><button className="button primary">Save changes</button><button type="button" className="danger-btn" onClick={() => setConfirm({ title: 'Cancel this class?', message: `${item.title} will be hidden from members and removed from normal booking.`, action: () => cancelClass(item._id), label: 'Cancel class' })}>Cancel class</button></div></form>) : <Empty text="No classes match that search." link="Browse classes" />}</div></article>
    {confirm && <ConfirmModal title={confirm.title} message={confirm.message} confirmLabel={confirm.label} onConfirm={confirm.action} onCancel={() => setConfirm(null)} />}
  </section>;
}
