...
export default function AdminClassManager({ classes, trainers, user, trainerName, readOnly = false, onUpdate, onCancel, onAttendance }) {
  return <article className="panel class-manager">
    ...
    <div className="managed-classes">{classes.length ? classes.map(item => <form key={item._id} data-id={item._id} onSubmit={onUpdate}>
      ...
      <div className="attendance-box"><span className="form-kicker">ROSTER</span>{item.attendees?.length ? item.attendees.map(member => <div className="attendance-row" key={member._id}><div><b>{member.name}</b><span>{member.email}</span></div><strong>{attendanceFor(item, member._id) || 'booked'}</strong><button type="button" disabled={readOnly} onClick={() => onAttendance(item._id, member._id, 'attended')}>Attended</button><button type="button" disabled={readOnly} onClick={() => onAttendance(item._id, member._id, 'missed')}>Missed</button></div>) : <p className="muted-line">No booked members yet.</p>}{Boolean((item.waitlist || []).length) && <div className="waitlist-mini"><b>Waitlist</b>{(item.waitlist || []).map(member => <span key={member._id}>{member.name}</span>)}</div>}</div>
      <div className="manager-actions"><button className="button primary" disabled={readOnly}>{readOnly ? 'Demo read-only' : 'Save changes'}</button><button type="button" className="danger-btn" disabled={readOnly} onClick={() => onCancel(item._id)}>Cancel class</button></div>
