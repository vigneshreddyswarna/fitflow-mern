export default function AdminClassForm({ user, trainers, onSubmit, readOnly = false }) {
  return <form className="panel admin-form" onSubmit={onSubmit}>
    ...
    <button className="button primary full" disabled={readOnly}>{readOnly ? 'Demo read-only' : 'Publish class'}</button>
  </form>;
}
